...
async function loadFirebaseClips() {
  const hasClipList = document.querySelector("#clipList");
  const hasClipDetail = document.querySelector("#clipDetail");

  if (!hasClipList && !hasClipDetail) return;

  // clip-detail.html: 해당 클립 1개만 읽기
  if (!hasClipList && hasClipDetail) {
    const params = new URLSearchParams(location.search);
    const clipId = params.get("id");
    if (!clipId) { renderClipDetail(); return; }
    try {
      const docSnap = await getDoc(doc(db, "clips", clipId));
      if (docSnap.exists()) {
        firebaseClips = [mapClipDoc(docSnap)];
      } else {
        firebaseClips = [];
      }
      renderClipDetail();
    } catch (error) {
      console.error("Firestore 클립 상세 불러오기 실패:", error);
      renderClipDetail();
    }
    return;
  }
  ...
  // clips.html: 전체 로드
  try {
    const clipsQuery = query(collection(db, "clips"), orderBy("createdAt", "desc"));
    const snapshot = await getDocs(clipsQuery);
    firebaseClips = snapshot.docs.map(mapClipDoc);
    renderClips();
  } catch (error) {
...
async function loadFirebasePeople() {
  ...
  try {
    ...
    firebasePeople = snapshot.docs.map(mapPersonDoc);
    renderPeople();
  } catch (error) {
...
function renderClips() {
  ...
  if (result.length === 0) {
    clipList.innerHTML = `<div class="empty-message">검색 결과가 없습니다.</div>`;
    return;
  }

  let clipsHtml = result.map((clip) => {
    return `
...
function renderPeople() {
  ...
  if (result.length === 0) {
    peopleList.innerHTML = `<div class="empty-message">검색 결과가 없습니다.</div>`;
    return;
  }

  let peopleHtml = result.map((person) => {
    return `
...
async function loadFirebaseWorldcups() {
  ...
  try {
    const worldcupQuery = query(collection(db, "worldcups"), orderBy("createdAt", "desc"));
    const snapshot = await getDocs(worldcupQuery);

    const newWorldcups = snapshot.docs.map((docItem) => {
      const data = docItem.data();
      return {
        id: docItem.id,
        title: data.title || "제목 없음",
    ...
    firebaseWorldcups = newWorldcups;

    renderWorldcups();
  } catch (error) {
    console.error("월드컵 불러오기 실패:", error);
    worldcupList.innerHTML = `<div class="empty-message">월드컵을 불러오지 못했습니다.</div>`;
  }
}
...
function renderWorldcups() {
  ...
  if (result.length === 0) {
    worldcupList.innerHTML = `<div class="empty-message">등록된 월드컵이 없습니다.</div>`;
    return;
  }

  let worldcupsHtml = result.map((worldcup) => {
  ...
  worldcupList.innerHTML = worldcupsHtml;
}
...
async function initAdminPage() {
  ...
  if (!currentUser) {
    showAdminAccessMessage(
      "로그인이 필요합니다.",
      "관리자 페이지는 SOOP 로그인 후 이용할 수 있습니다."
    );
  ...
  if (!soopUser) {
    showAdminAccessMessage(
      "SOOP 로그인이 필요합니다.",
      "관리자 페이지는 SOOP 계정으로 로그인 후 이용할 수 있습니다."
    );
    return;
  }
  ...
  try {
    const res = await fetch("/api/admin-verify", {
      ...
      body: JSON.stringify({ soopId: soopUser.soopId })
    });
    ...
    if (res.ok) {
      ...
      showAdminAccessMessage(
        "관리자 권한이 없습니다.",
        `SOOP ID: ${soopUser.soopId} — 관리자로 등록되지 않은 계정입니다.`
      );
    }
  } catch (error) {
    console.error("관리자 인증 실패:", error);
    showAdminAccessMessage("인증 오류", "관리자 인증 중 오류가 발생했습니다.");
  }
}
